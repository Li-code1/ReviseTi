...
6. `supabase/migrations/0006_lessons_expanded.sql` — aprofunda o conteúdo (campo `content`) das 25 aulas: mais teoria, mais exemplos de código, seção de "erros comuns" e checklist em cada uma. Só faz `UPDATE` por `slug` — não mexe em tópicos, perguntas, progresso ou qualquer outra tabela. Idempotente.
